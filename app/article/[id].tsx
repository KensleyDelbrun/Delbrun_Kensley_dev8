...
export default function ArticleDetailScreen() {
  ...
  const loadArticle = async () => {
    ...
    if (offline === 'true') {
      const offlineArticle = getOfflineArticleById(id!);
      if (offlineArticle) {
  ...
  const checkIfSaved = () => {
    if (article) {
      const saved = isArticleSavedOffline(article.id);
      setIsSaved(saved);
    }
  };
  ...
  const handleSaveToggle = async () => {
    ...
    if (isSaved) {
      ...
      if (result.success) {
        ...
        Alert.alert(
          t('Article sauvegardé', 'Atik sovgade'),
          t('Disponible hors ligne', 'Disponib san koneksyon')
        );
      }
    }
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>
            {t('Article introuvable', 'Atik pa jwenn')}
          </Text>
          <Text style={styles.errorSubText}>
            {t('Vérifiez votre connexion ou réessayez plus tard.', 'Tcheke koneksyon ou epi eseye ankò pita.')}
          </Text>
        </View>
      </SafeAreaView>
    );
